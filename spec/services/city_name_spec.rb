...
RSpec.describe CityName  do
  ...
  context 'fill table with data' do
    ...
    it 'has to be correct with load from site' do
      test_cities = instance_double(described_class)

        allow(CityName).to receive(:new).and_return(test_cities)
        allow(test_cities).to receive(:values_from_site).and_return(
          ["Aachen1", "Aalborg", "Aalesund", "Aare", "Aarhus", "Aba", "Abadan", "Abakan", "Abbotsford", "Abeokuta2", "Aberdeen"]
        )
      load_from_site
